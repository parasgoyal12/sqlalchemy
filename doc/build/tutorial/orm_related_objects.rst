...
available, or when using alternative concurrency patterns such as `asyncio
...
SELECT statement using the :meth:`_sql.Select.options` method.   Each loader
strategy object adds some kind of information to the statement that will
be used later by the :class:`_orm.Session` when it is deciding how various
attributes should be loaded and/or behave when they are accessed.

.. seealso::

    :ref:`loading_toplevel` - Reference documentation for loader strategies
